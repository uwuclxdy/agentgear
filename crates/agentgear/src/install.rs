...
/// One agent's uninstall slice; a failure here never aborts the fan-out.
///
/// The marker is agentgear's own state — cleared even for a skipped agent, so an
/// explicit uninstall never orphans one. A FAILED remove keeps its marker (the
/// install is still live; the next uninstall or self_heal picks it back up). The
