...
impl AgentBackend for KiloBackend {
    ...
    fn probe(&self, plugin: &Plugin, scope: &Scope) -> Result<BackendState> {
        // Ownership is defined by our mcp server keys; `probe_mcp` returns Healthy —
        ...
        // (github unsupported, path install-only), mirroring the claude probe.
...
fn check_mcp_registered(servers: &[McpServer], root: Option<&Value>) -> DoctorCheck {
    ...
    if missing.is_empty() {
        ...
        DoctorCheck {
            name,
            status: CheckStatus::Fail {
                problem: format!("mcp server(s) not under `mcp` in kilo.json: {}", missing.join(", ")),
